...
//TODO: HACK: This should not be available outside the engine.
...
{
	//KINFO("Game updated with delta time: %f", delta_time);
...
{
	//KINFO("Game rendered with delta time: %f", delta_time);
	return true;
}

void game_on_resize(game* game_inst, u32 width, u32 height)
{
	//KINFO("Game resized to: %dx%d", width, height);
}