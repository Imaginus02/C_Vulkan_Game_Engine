...
void vulkan_fence_create(vulkan_context* context, b8 create_signaled, vulkan_fence* fence) {

	fence->is_signaled = create_signaled;

	VkFenceCreateInfo fence_create_info = {VK_STRUCTURE_TYPE_FENCE_CREATE_INFO};
	fence_create_info.flags = fence->is_signaled ? VK_FENCE_CREATE_SIGNALED_BIT : 0;
...
b8 vulkan_fence_wait(vulkan_context* context, vulkan_fence* fence, u64 timeout_ns) {
	if (fence->is_signaled) {
		//KWARN("Fence is already signaled, no need to wait on it");
		return true;
	}
	...
	case VK_ERROR_OUT_OF_DEVICE_MEMORY:
		...
		break;
	default:
		KERROR("vk_fence_wait - Unknown error");
		break;
	}
	return false;
}
