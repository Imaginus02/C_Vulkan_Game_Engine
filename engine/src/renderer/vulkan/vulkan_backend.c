...
b8 vulkan_renderer_backend_initialize(renderer_backend* backend, const char* application_name) {
	...
	app_info.pEngineName = "First Engine";
	...
	darray_push(required_extensions, &VK_KHR_SURFACE_EXTENSION_NAME); // General surface extension
    ...
    vulkan_renderpass_create(
        ...
        0.0f, 0.0f, (f32)context.framebuffer_width, (f32)context.framebuffer_height,
        0.0f, 0.0f, 0.2f, 1.0f, // Dark blue background
        1.0f,
        0);

    // Swapcahin framebuffers
...
void vulkan_renderer_backend_shutdown(renderer_backend* backend) {
    // Wait for the device to finish before destroying it
    ...
    for (u32 i = 0; i < context.swapchain.image_count; ++i) {
        if (context.swapchain.framebuffers[i].handle) {
            vulkan_framebuffer_destroy(&context, &context.swapchain.framebuffers[i]);
            context.swapchain.framebuffers[i].handle = 0;
        }
    }
...
void vulkan_renderer_backend_on_resized(renderer_backend* backend, u16 width, u16 height) {
    KINFO("Resizing Vulkan renderer to %dx%d...", width, height);
...
b8 vulkan_renderer_backend_begin_frame(renderer_backend* backend, f32 delta_time) {
    ...
    //Check if reacreating swap chain and boot out
    ...
    if (context.framebuffer_size_generation != context.framebuffer_size_last_generation) {
        ...
        // If the swapchain recreation failed (e.g. due to the window being minimized)
    ...
    if (!vulkan_fence_wait(
            ...
            0xffffffffffffffff)) {
        KWARN("In-flight fence wait failed.");
        return false;
    }

    // Acquire the swapchain next image index
    if (!vulkan_swapchain_acquire_next_image_index(
            ...
            0,
            &context.image_index)) {
        KWARN("Failed to acquire next image.");
        return false;
    }
...
b8 vulkan_renderer_backend_end_frame(renderer_backend* backend, f32 delta_time) {
    ...
    // Make sur the previous frame is not using this image
    ...
    if (result != VK_SUCCESS) {
        KERROR("vkQueueSubmit failed: %s", vulkan_result_string(result, true));
        return false;
    }
...
void regenerate_framebuffers(renderer_backend *backend, vulkan_swapchain *swapchain, vulkan_renderpass *renderpass) {
    for (u32 i = 0; i < swapchain->image_count; ++i) {
        // TODO: make this dynamix based on the currently configured attachments
        u32 attachment_count = 2;
        VkImageView attachments[] = {swapchain->views[i], context.swapchain.depth_attachment.view};
