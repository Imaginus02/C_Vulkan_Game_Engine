...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
	...
	// Two frame being rendered to and another one being presented.

	// CHoose a swap surface format.
	b8 found = false;
	for (u32 i = 0; i < context->device.swapchain_support.format_count; ++i) {
		VkSurfaceFormatKHR format = context->device.swapchain_support.formats[i];
		// Preferred format : 32-bit BGRA with sRGB color space.
	...
	if (!found) {
		// If preferred format is not available, choose the first available format.
	...
	if (!vulkan_device_detect_depth_format(&context->device)) {
		...
		KFATAL("Failed to detect depth format!");
	}
